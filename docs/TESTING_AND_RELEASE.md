...
**Status:** Binding verification and release-acceptance procedure for the Android-first target
release. It defines required evidence, not current implementation status.
...
| `QG-03` Native integration         | The exact Android binary proves capability, bridge, runtime, Segmentation, persistence, save/share, checksum, and terminal-result behavior.                | Physical-device integration report; Native/ML and Mobile leads                                                                                                         |
...
- model-manifest checksum, runtime, memory, delegate, and dimension validation;
- generated-asset metadata, immutable IDs, SHA-256, local URI ownership, persistence transaction,
  deletion, and orphan cleanup;
- safe error mapping and retryability for every contract error code;
- local safety allow/block decisions, fixed-negative-prompt injection, raw-prompt redaction, and
  ruleset rollback protection; and
...
- `allowed-generation-1.0` and `invalid-request-1.0`;
- `blocked-prompt-1.0`;
- `cancel-generation-1.0`;
- `model-incompatible-1.0`;
- `segmentation-failure-1.0`;
- `asset-storage-failure-1.0`; and
- `share-unavailable-1.0`.
...
Run these tests in an Android development build during implementation and repeat the release gate
against the exact release-candidate binary on representative physical devices. Expo Go cannot load
the custom inference native module and is not a valid native test environment.
...
2. `prepareModel` verifies the selected `ModelManifest`, exact artifact SHA-256, runtime,
   ...
   reach inference.
3. An allowed request crosses the bridge only after moderation, receives exactly one admission,
   emits the contract-defined progress sequence, and resolves exactly once.
4. The native runtime uses the selected Plan A/B/C artifact and fixed safety negative prompt; bridge
   captures contain no blocked raw prompt or implementation-secret detail.
...
- empty/invalid input, allowed moderation, generic blocked response, and edit/resubmit;
- all exact progress stages, cancellation, success, preview transparency, regenerate, and retry;
- save, native share success/unavailable behavior, local gallery reuse, deletion, and reset;
...
prompt, bridge serialization, or patch channel. This is separate from functional QA and follows the
...
- blocked decisions occur before generation and expose only `PROMPT_BLOCKED` with generic safe
  copy;
- blocked prompts never enter a generation payload, progress stream, gallery, log, telemetry,
  screenshot, or evidence package;
- allowed requests receive the fixed safety negative prompt exactly once at the native boundary;
- ruleset signatures, expiry, rollback protection, atomic activation, last-valid fallback, and
  offline bundled baseline behave as specified;
- sync/network failure never blocks offline generation or weakens the last valid local ruleset; and
...
enough to expose to participants. Distribute the same identified release candidate through Google
Play Internal or Closed Testing to focused users aged 16–30 across the supported-floor device
matrix, consistent with [Roadmap `W4-03`](./ROADMAP.md#week-4-integration-and-wide-testing).
...
- Expo/React Native dependencies pass `npx expo install --check`; the
  [Expo SDK 57 reference](https://docs.expo.dev/versions/v57.0.0/) lists Android compile/target SDK
  36, and the release owner separately rechecks then-current Play policy in Play Console before
  submission;
...
- [ ] `npm ci`, `npx expo install --check`, typecheck, lint, unit tests, format check, and every
      checked-in native/device suite pass with complete logs.
- [ ] Every producer, consumer, and integrated path passes every applicable contract fixture.
- [ ] Native integration passes in the custom Android binary; no Expo Go, web, emulator, desktop,
      or mock result is counted as physical-device inference evidence.

...
**Status:** Implementation policy for v1.
...
1. The on-device Safety filter evaluates the normalized prompt before any generation work. A blocked decision does not cross the generation boundary, does not create an asset or gallery item, and receives the generic, non-revealing user response required by [Integration Contracts](./INTEGRATION_CONTRACTS.md#safety-decision-contract).
2. Every admitted generation uses the fixed safety negative prompt as a second, always-on model-conditioning layer. It is defense in depth, not a substitute for input filtering.
3. The filter and negative prompt are reviewed together whenever rules or model configuration change. The filter's matched rule, token, and internal scoring are never displayed to users.
...
The safety negative prompt is bundled with the selected local model configuration and applied to every admitted generation. It is not displayed as editable text, exposed as a user preference, or user-editable. Changes to it require the same safety review and release process as a filtering-logic change; a blocklist update cannot alter it.

## Local Blocklist

The app ships with a reviewed, signed local baseline blocklist so that safety enforcement works without connectivity from first launch. Each app build declares the baseline revision as its minimum accepted revision. Rules are versioned and stored as app-local safety configuration. A candidate ruleset is validated before activation; invalid, incomplete, unreadable, or below-floor rules never replace the last valid ruleset.
...
When connectivity is available, the app may check for a blocklist update in the background. The check must not delay capability checks, prompt moderation, generation, save, share, or gallery access.

Before activation, an update must include its revision, `issuedAt`, `expiresAt`, and build-compatibility declaration, then pass authenticity verification against the pinned signing authority, integrity validation, compatibility validation, and rollback protection. The app rejects an update that is older than the build's minimum revision, expired, invalid, incompatible with the installed build, or no newer than the retained `maximumAcceptedRevision`, even when it is otherwise authentic.

Installation is atomic: the current valid ruleset remains active until every check succeeds. The app keeps `maximumAcceptedRevision` and the corresponding last-valid safety configuration outside the user-content reset domain. An in-app reset atomically rebuilds safety configuration from the signed bundled baseline and re-applies a retained newer valid ruleset when one exists; it never activates a revision below `maximumAcceptedRevision`. A platform app-data clear or uninstall may start again from the installed build's signed baseline, but later updates still cannot be below that build's minimum revision.

Update failure, an unavailable network, an invalid package, an expired package, or a rejected rollback leaves the last valid ruleset active; when no retained update exists, the signed bundled baseline remains active. Neither outcome blocks offline generation. A rules-only update can close a rule-coverage gap; a filtering-logic or fixed-negative-prompt change requires a reviewed app update.

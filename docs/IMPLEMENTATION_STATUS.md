...
| **F-013**  | One-person InstantID sticker provider | ✅ Completed | `backend/app/providers/instantid_provider.py`                          |
...
| **F-018**  | Generic image validation   | ✅ Completed | `LocalImageValidator` checks format, dimensions, blank images, and returns reason codes. |
...
- Local generation requires the complete pinned model tree documented in
  `models/README.md`, a CUDA-capable worker, and a separately reviewed chibi LoRA.
- Multi-person generation is intentionally not implemented.

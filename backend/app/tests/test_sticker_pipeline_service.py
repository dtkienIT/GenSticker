...
@pytest.mark.asyncio
async def test_completed_pack_uses_storage_urls_for_database_history(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "OPENAI_API_KEY", "test-key")
    ...
    def fake_upload(_data: bytes, file_name: str, _content_type: str) -> str:
        uploaded_names.append(file_name)
        return f"https://storage.test/{file_name}"

    def fake_save(**kwargs: object) -> dict[str, str]:
        saved.update(kwargs)
        return {"id": "pack-test"}
